...
export function middleware(request: NextRequest) {
  ...
  if (request.headers?.get("/app/panel_admin/panel_admin.ts")?.includes("admin")) {
    return NextResponse.rewrite("host/", { status: 301, headers: { location: "/panel_admin/panel_admin" } })
  }
}
